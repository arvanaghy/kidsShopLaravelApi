    ...
    public function list_past_orders_products(Request $request, $order)
    {
        try {
            ...
            if ($userResult) {
                ...
                return response()->json([
                    "message" => "با موفقیت انجام شد",
                    "result" => DB::table('AV_RFactorForooshKala_VIEW')->where('CodeDoreMali', $this->financialPeriod)->where('CodeFactorForoosh', $order)->where('CCode', $userResult->Code)->paginate(12)
                ], 201);
            }
            return response()->json([
                "message" => "کاربری با این توکن یافت نشد",
